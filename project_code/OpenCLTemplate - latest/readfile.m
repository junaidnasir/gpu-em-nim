...
fidp = fopen(strcat(filepath,'\SIZE','.jd'),'r','l');
if fidp==-1
    display('Error');
    return
end
SIZE=fread(fidp,1,'int');
fclose(fidp);
...
Eincident=fread(fidp,maxTime,'float');
fclose(fidp);
...
Etransmitted=fread(fidp,maxTime,'float');
fclose(fidp);
