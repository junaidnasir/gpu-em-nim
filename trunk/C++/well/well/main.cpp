...
void main()
{
    ...
    // Temp Variable
		...
		for (int qTime=0; qTime<(maxTime-1); qTime++)
		{
			...
			snapshot.open(basename, ios::out|ios::binary);
			for (mm = 0; mm < SIZE; mm++)
	...
	double a[3]={-0.00000234,0.123123123,0.123};
	char basename2[80] = "Eincident";
	/*sprintf(filename, "%s.bin", basename2);
	snapshot=fopen("feincident.bin", "wb");
	//for (int mm = 0; mm < SIZE; mm++)
	//fprintf(snapshot, "%g\n", FEincident[mm].re());
	fwrite(&Etransmitted,8*maxTime,1,snapshot);
	fclose(snapshot);*/

	//-----new
	snapshot.open(basename2, ios::out|ios::binary);
	snapshot.write((char *)&Eincident,(sizeof(double)*maxTime));
	snapshot.close();
	//cin>>a;
