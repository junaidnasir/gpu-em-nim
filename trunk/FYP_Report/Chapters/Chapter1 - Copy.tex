...
\begin{figure}[htbp]
	\centering
		\includegraphics{Figures/Software_Arch.png}
		\rule{35em}{0.5pt}
	\caption[An Electron]{Software Archet (artist's impression).}
...
Code snippets can be inserted with the listings command. You can also specify the language for which you want to highlight your code. Here is some matlab code:
\lstset{language=Matlab, commentstyle=\color{gray}, stringstyle=\color{red!60!black}}
...
\subsection{Index}
Key words and terms in the thesis can be indexed with the \verb|\index| directive. Indexed entries appear in the Index section. The \verb|\index| command takes a string as argument. This string is the label under which index entry is listed. Following paragraph as several words indexed.
